...
# 4 - Conflicts in modules_functions variable (__module_load only)
...
function __module_load
    ...
    # 5. Parse __module_functions, handle conflicts
    # 6. Write module infos into a file in metadata/modulebasename.fish
    # 7. Append functions to a global variable if no conflict found
    ...
    if not set -q __module_name
        ...
        pr_warn modules_loader "__module_load: Module $argv[1] did not set one of the following property: __module_name, __module_version, __module_description, __module_functions"
    ...
    for fnt in $__module_functions
        ...
        for fn in $modules_functions
            if test "$fnt" = "$fn"
                ...
                set fatal_exception_function $fn
                set abort true
                break
            end
        end
    ...
    ############ BEGIN exception handling for module ###############
    if test "$abort" = true
        # Clean back up all set functions except for
    ...
    ############ END exception handling for module ###############
...
" >metadata/$module_basename
    ...
    # Append functions to a global variables
