...
function __module_load
    ...
    set -l module_events_quoted (
    for modev in $__module_events
        string replace "$modev" "\"$modev\"" $modev
    end
    )
    set -l module_functions_quoted (
    for modfn in $__module_functions
        string replace "$modfn" "\"$modfn\"" $modfn
    end
    )
    echo "\
